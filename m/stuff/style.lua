...
local function button_hover_scale(node, target, time)
	gui.animate(node, "scale", target, gui.EASING_OUTQUART, time)
end

local function button_tap_anim(node, tap_scale, start_scale)
	gui.animate(node, gui.PROP_SCALE, tap_scale, gui.EASING_OUTQUART, 0.2)
end

local SCALE_BTN_TO = vmath.vector3(1, 1, 1) - vmath.vector3(0.1, 0.1, 0)
...
M["button"] = {
	...
	on_hover = function(self, node, state)
		local scale_to = state and SCALE_BTN_TO or self.start_scale
		button_hover_scale(node, scale_to, M.button.HOVER_TIME)
	end,
	...
	on_click = function(self, node)
		...
		button_hover_scale(node, self.start_scale, M.button.HOVER_TIME)
		if self.sound then
			sound.play(self.sound)
		end
		--sound.play("loader:/sounds#btn_click")
